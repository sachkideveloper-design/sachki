...
async function subirFotoPerfil(usuarioId, archivo) {
    if (!archivo) return { exito: false, error: 'No se seleccionó ninguna foto' };
    if (!archivo.type.startsWith('image/')) return { exito: false, error: 'El archivo debe ser una imagen' };
    if (archivo.size > 5 * 1024 * 1024) return { exito: false, error: 'La imagen no puede pesar más de 5MB' };

    try {
        const ref = window.storage.ref().child('fotos_perfil/' + usuarioId);
        await ref.put(archivo);
        const url = await ref.getDownloadURL();
        await updateDoc(doc(getDB(), 'usuarios', usuarioId), { foto_url: url });
        return { exito: true, url };
    } catch (err) {
        console.error('Error al subir foto de perfil:', err);
        return { exito: false, error: 'No se pudo subir la foto. Intenta de nuevo.' };
    }
}

// ─── CÓDIGO DE ACCESO QR (check-in en recepción) ──────────────────────────────
// El QR se regenera cada pocos segundos (ver GYM_CONFIG.duracionCodigoAccesoMs)
// y se invalida en cuanto se usa una vez, para que un screenshot compartido
// deje de servir casi de inmediato.
...
async function regenerarCodigoAcceso(usuarioId) {
...
// Usado por el lector de recepción (kiosko.html). Valida el QR escaneado,
// registra el acceso en la colección "accesos" y de una vez invalida el
// token para que no se pueda reusar (ni siquiera dentro de su ventana de tiempo).
async function registrarAcceso(datosEscaneados) {
    let payload;
    try {
        payload = JSON.parse(datosEscaneados);
    } catch (err) {
        return { exito: false, motivo: 'Código QR inválido o dañado' };
    }
    const { uid, tok } = payload;
    if (!uid || !tok) return { exito: false, motivo: 'Código QR inválido o dañado' };

    const ref = doc(getDB(), 'usuarios', uid);
    const snap = await getDoc(ref);
    if (!snap.exists) return { exito: false, motivo: 'Usuario no encontrado' };
    const usuario = { id: uid, ...snap.data() };

    const registrar = async (resultado, motivo) => {
        await addDoc(collection(getDB(), 'accesos'), {
            usuarioId: uid,
    ...
    if (usuario.accessToken !== tok) {
        await registrar('rechazado', 'Código ya usado o inválido');
        return { exito: false, motivo: 'Código ya usado o inválido', usuario };
    }
    if (!usuario.accessTokenExpira || Date.now() > usuario.accessTokenExpira) {
        await registrar('rechazado', 'Código expirado');
        return { exito: false, motivo: 'Código expirado, pide que lo regenere', usuario };
    }
    if (!membresiaVigente(usuario)) {
        await updateDoc(ref, { accessToken: null, accessTokenExpira: null });
        await registrar('rechazado', 'Membresía vencida');
        return { exito: false, motivo: 'Membresía vencida', usuario };
    }

    // Válido: invalidar el token de inmediato (uso único) y registrar el acceso
    await updateDoc(ref, { accessToken: null, accessTokenExpira: null });
    await registrar('permitido', null);
    return { exito: true, usuario };
}

async function obtenerAccesosDelDia(fecha) {
    const q = query(collection(getDB(), 'accesos'), where('fecha', '==', fecha || obtenerFechaHoy()));
    const snap = await getDocs(q);
    return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}
