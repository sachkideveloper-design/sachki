...
function setupEventListeners() {
 ...
 if (fotoPerfilInput) fotoPerfilInput.addEventListener('change', async () => {
 const archivo = fotoPerfilInput.files[0];
 fotoPerfilInput.value = '';
 if (!archivo || !window._usuarioActual) return;
 const avatar = document.getElementById('fotoPerfilAvatar');
 if (avatar) avatar.style.opacity = '0.5';
 const r = await storage.subirFotoPerfil(window._usuarioActual.id, archivo);
 if (r.exito) {
 window._usuarioActual.foto_url = r.url;
 pintarPerfil(window._usuarioActual);
 } else {
 alert(r.error);
...
async function cambiarSeccion(nombreSeccion) {
 ...
 if (nombreSeccion !== 'access') detenerCodigoAcceso();
 const mapa = {
 ...
 access: async () => { document.getElementById('accessSection').classList.add('active'); await iniciarCodigoAcceso(); },
...
function pintarPerfil(usuario) {
 ...
 seccion.querySelector('.card').innerHTML = `
     <div style="display:flex;align-items:center;gap:14px;margin-bottom:20px;padding-bottom:16px;border-bottom:1px solid rgba(255,255,255,0.06);">
         <div id="fotoPerfilAvatar" style="width:64px;height:64px;border-radius:50%;background:${fotoUrl ? `url('${fotoUrl}') center/cover` : 'var(--border-color)'};flex-shrink:0;display:flex;align-items:center;justify-content:center;cursor:pointer;border:2px solid var(--primary-color);" title="Cambiar foto">
             ${fotoUrl ? '' : '<span style="font-size:22px;">👤</span>'}
         </div>
         <div>
             ...
             <button id="fotoPerfilBtn" class="btn" style="margin-top:6px;padding:4px 10px;font-size:11px;">${fotoUrl ? 'Cambiar foto' : 'Agregar foto'}</button>
         </div>
     </div>
 ...
 const abrirSelector = () => document.getElementById('fotoPerfilInput')?.click();
 seccion.querySelector('#fotoPerfilAvatar')?.addEventListener('click', abrirSelector);
 seccion.querySelector('#fotoPerfilBtn')?.addEventListener('click', abrirSelector);
}
...
// ─── MI CÓDIGO DE ACCESO (QR rotativo para check-in) ──────────────────────────

let intervaloCodigoAcceso = null;

async function iniciarCodigoAcceso() {
 detenerCodigoAcceso(); // por si quedó uno corriendo de antes
 const usuario = window._usuarioActual;
 const estadoEl = document.getElementById('accessQrEstado');
 const wrapEl = document.getElementById('accessQrWrap');
 if (!usuario) return;

 // Si es miembro (rol 'usuario') y su membresía ya venció, no mostramos un QR
 // que de todas formas el lector va a rechazar — mejor avisarle de una vez.
 if (usuario.rol === 'usuario' && !storage.membresiaVigente(usuario)) {
     if (wrapEl) wrapEl.style.display = 'none';
     if (estadoEl) estadoEl.innerHTML = '⚠️ Tu membresía está vencida.<br>Acércate a recepción para renovarla.';
     return;
 }
 if (wrapEl) wrapEl.style.display = 'flex';

 const dibujarNuevoCodigo = async () => {
     const r = await storage.regenerarCodigoAcceso(usuario.id);
     const canvas = document.getElementById('accessQrCanvas');
     if (!canvas || typeof QRCode === 'undefined') return;
     const payload = JSON.stringify({ uid: usuario.id, tok: r.token });
     QRCode.toCanvas(canvas, payload, { width: 220, margin: 1 }, err => {
         if (err) console.error('Error generando QR:', err);
     });
     if (estadoEl) estadoEl.textContent = 'Se renueva automáticamente cada rato ✓';
 };

 await dibujarNuevoCodigo();
 const duracion = (window.GYM_CONFIG && window.GYM_CONFIG.duracionCodigoAccesoMs) || 30000;
 intervaloCodigoAcceso = setInterval(dibujarNuevoCodigo, duracion);
}

function detenerCodigoAcceso() {
 if (intervaloCodigoAcceso) {
     clearInterval(intervaloCodigoAcceso);
     intervaloCodigoAcceso = null;
 }
}
